...
| P0 | Configurable runtime entrypoint | Start multi-protocol gateway from config instead of source edits; TCP TLS, QUIC, and Origin allowlists are configurable |
...
- HTTP CORS policy shape and finer protocol security defaults.
...
- Policy tests for HTTP CORS and protocol security defaults.
...
- Configurable runtime entrypoint is complete for JSON config, environment
  ...
  certificate/key config, and WebSocket/gRPC-Web Origin allowlists.
- mTLS configuration, certificate reload, HTTP CORS policy configuration, and
  finer protocol security defaults remain part of the security-baseline milestone.
