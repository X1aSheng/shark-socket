...
func TestK8sManifestSemantics(t *testing.T) {
	...
	configMap := readFile(t, "../../deploy/k8s/configmap.yaml")
	...
	// Env vars are wired from the ConfigMap (envFrom), not duplicated inline.
	assertContains(t, deployment, "envFrom:")
	assertContains(t, deployment, "configMapRef:")
	assertContains(t, deployment, "shark-socket-config")
	assertContains(t, configMap, "SHARK_TCP_ADDR: \"0.0.0.0:18000\"")
