...
- 不内置完整 MQTT Broker，由外部 [shark-MQTT](https://github.com/X1aSheng/shark-MQTT) 通过数据契约互通。
...
- **可观测优先**：关键路径内置 metrics / trace / log；Prometheus 指标静态预注册防基数爆炸；提供 `/healthz`、`/readyz` 与 `sessions_active` 等会话指标。
- **benchmark 驱动**：性能优化以基准测试与 pprof 热点证据为前提，禁止凭直觉改结构。
- **编译期验证**：`var _ Interface = (*Impl)(nil)` 接口满足检查，关键约束用类型系统表达。
- **安全内建**：TLS 证书热加载、mTLS、DTLS、接受速率限制、写超时、非 root 容器、只读根文件系统、Drop ALL capabilities。
...
| 单元测试（26 套件） | `go test ./...` | ✅ |
| 竞态检测 | `go test -race ./...` | ✅ |
| 覆盖率（70% 门槛） | `go run scripts/run_tests.go -mode cover` | ✅ 75.2% |
